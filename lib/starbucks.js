...
class Client {
  ...
  activateCard(cardId) {
    ...
    return this._authenticatedRequest({
      ...
      data: {
        countryCode: "FR",
        deviceId: faker.fake("{{random.uuid}}"),
        hardwareId: faker.fake("{{random.uuid}}"),
        actionTime: Moment().utc(),
        starbucksCardNumber: cardId,
        actionCode: "ADSC"
      }
    })
      .then(response => {
  ...
  getNearbyStores(latitude, longitude, radius, limit) {
    const url =
      ...
      QueryString.stringify({
        ...
        offset: 0,
        ignore:
          "regularHours,timeZoneInfo,extendedHours,brandName,ownershipTypeCode,operatingStatus,regulations,serviceTime,xopState,currency,address.countrySubdivisionCode,hoursNext7Days.holidayCode",
        access_token: this.access_token
      });
    return this._httpClient({
      method: "GET",
  ...
  getRewards() {
    return this.getProfile().then(response => {
      let rewards = _.get(response, "rewardsSummary");
  ...
  getCards() {
    return this.getProfile().then(response => {
      let starbucksCards = _.get(response, "starbucksCards");
  ...
  getBarcode(cardNumber) {
    console.log(cardNumber);
    return this._authenticatedRequest({
      method: "get",
      url: "barcode/starbuckscard/" + cardNumber,
  ...
  place_order(details, card_id) {
    ...
    return this._authenticatedRequest({
      method: "post",
      url:
        `me/stores/${details.store.storeNumber}/orderToken/${details.orderToken}/submitOrder?` +
        QueryString.stringify({
          market: "US",
          locale: "en-US"
        }),
